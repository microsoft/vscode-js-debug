...
export class BrowserTargetManager implements IDisposable {
   ...
   * the UI toggle is only exposed on the extensions WebUI page, so this
   * navigates the launcher's initial blank tab there (or opens a tab when
   * there is none) and leaves the page open — visible confirmation for the
   * developer that their extension loaded. Best-effort: failures are logged
   * and ignored.
   */
  public async enableExtensionDeveloperMode(): Promise<void> {
    try {
      ...
      const blank = targets?.targetInfos.find(
        t => t.type === BrowserTargetType.Page && (t.url === 'about:blank' || t.url === ''),
      );

      const targetId = blank?.targetId;
      let navigated = false;
      if (targetId) {
        const attached = await this._browser.Target.attachToTarget({ targetId, flatten: true });
        if (attached) {
          const session = this._connection.createSession(attached.sessionId);
          try {
            navigated = !!(await session.Page.navigate({ url: 'chrome://extensions/' }));
            if (navigated) {
              await this.toggleDeveloperModeOn(session);
            }
          } finally {
            this._connection.disposeSession(attached.sessionId);
            await this._browser.Target.detachFromTarget({ sessionId: attached.sessionId });
      ...
      // no blank tab to reuse (or it refused the WebUI navigation): open the
      // page in its own tab instead, and keep it open
      if (!navigated) {
        const created = await this._browser.Target.createTarget({ url: 'chrome://extensions/' });
        if (!created) {
          return;
        }
  ...
  private async toggleDeveloperModeOn(session: Cdp.Api): Promise<void> {
