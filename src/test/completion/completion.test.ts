...
describe('completion', () => {
  ...
  itIntegrates(
    'completes primitive when name clashes with template locals (#2399)',
    async ({ r }) => {
      const p = await r.launch(`
      <script>
        function foo() {
    ...
    `);
      ...
      expect(forTarget.targets.some(t => t.label === 'toLowerCase' || t.label === 'length')).to
        .equal(
          true,
          `expected string completions for target., got ${
            JSON.stringify(forTarget.targets.slice(0, 5))
          }`,
        );
    },
  );
