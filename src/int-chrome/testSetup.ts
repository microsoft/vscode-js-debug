...
function formLaunchArgs(launchArgs: IChromeLaunchConfiguration & Dictionary<unknown>, testTitle: string): void {
 ...
 */
export async function setup(context: IBeforeAndAfterContext | ITestCallbackContext, launchProps?: Partial<IChromeLaunchConfiguration>): Promise<ExtendedDebugClient> {
 ...
 *    - Best practise: The new best practise is to use the DefaultFixture when possible instead of calling this method directly
 */
export async function setupWithTitle(testTitle: string, launchProps?: Partial<IChromeLaunchConfiguration>): Promise<ExtendedDebugClient> {
    ...
    const debugClient = await testSupportSetup.setup({ type: 'pwa-chrome', patchLaunchArgs: args => patchLaunchArgs(args, testTitle), port });
