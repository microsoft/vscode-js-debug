...
puppeteerSuite('Stack Traces', TEST_SPEC, (suiteContext) => { // Need attach mode
    puppeteerTest('Stack trace is generated with no formatting', suiteContext, async (_context, page) => {
        await validateStackTrace({
            ...
            format: {},
            expectedFrames: [
                { name: '<anonymous>', line: 11, column: 9, source: { fileRelativePath: 'app.js' }, presentationHint: 'normal'},
                { name: 'evalCallback', line: 12, column: 7, source: { fileRelativePath: 'app.js' }, presentationHint: 'normal'},
                { name: '<anonymous>', line: 1, column: 1, source: { evalCode: true }, presentationHint: 'normal'},
