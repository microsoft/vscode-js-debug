...
suite('loaded sources', () => {
    ...
    testUsing('we receive events for js, ts, and eval sources', context => LaunchProject.launch(context, testSpec, {} as IChromeLaunchConfiguration, // TODO@rob
    ...
    testUsing('can get dynamic JavaScript file source', context => LaunchProject.launch(context, testSpec, {} as IChromeLaunchConfiguration,
        { registerListeners: client => client.on('loadedSource', args => onLoadedSource(<DebugProtocol.LoadedSourceEvent>args)) }), async launchProject => {
            await launchProject.pausedWizard.waitUntilPausedOnDebuggerStatement();
    ...
    testUsing('can get dynamic .html file source', context => LaunchProject.launch(context, testSpec, {} as IChromeLaunchConfiguration,
        { registerListeners: client => client.on('loadedSource', args => onLoadedSource(<DebugProtocol.LoadedSourceEvent>args)) }), async launchProject => {
            await launchProject.pausedWizard.waitUntilPausedOnDebuggerStatement();
